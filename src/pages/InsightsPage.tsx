...
import { Download, Link as LinkIcon, FileText, ArrowLeft, Cpu } from "lucide-react";
...
const InsightsPage: React.FC = () => {
  ...
  const [modelUsed, setModelUsed] = useState<string>("AI Model");
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const storedModelId = sessionStorage.getItem("analysisModel");
        ...
        if (storedData) {
          ...
          if (storedModelId) {
            const model = getModelById(storedModelId);
            if (model) {
              setModelUsed(model.name);
            }
          }
          ...
          if (!redditClientId || !redditClientSecret || (!hasOpenRouterKey && !hasGeminiKey)) {
          ...
          if (competitorName !== "Competitor") {
            ...
            const modelId = hasGeminiKey ? "google/gemini-pro" : "openai/gpt-4o";
            const result = await analyzeCompetitor(competitorName, "", "3", modelId);
            setData(result);
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      // Add footer with date and model info
      const currentDate = new Date().toLocaleDateString();
      doc.setFontSize(10);
      doc.text(`Generated by RoastRadar on ${currentDate} using ${modelUsed}`, 15, doc.internal.pageSize.height - 10);
